...
> Make real-time tactical decisions during live IPL match simulations, get Claude AI analysis on every call, and compete against fans nationwide on the leaderboard.
...
After each decision, **Claude AI** provides a 2-3 sentence tactical breakdown comparing your call to the captain's and historical trends.
...
3. If no one joins, **Claude AI steps in as your opponent** automatically
4. Both coaches make decisions for the same balls — highest scorer wins

---

### 4. 🎙️ AI Commentator
After each over, **Claude generates a Harsha Bhogle-style commentary summary**:
...
- Claude explains the counterfactual: "If you'd placed deep midwicket instead…"

---

### 6. 📊 Coach Report Card
Post-match personalised report generated by Claude:
...
> **Note:** AI features (tactical explanations, commentary, report card) fall back to pre-written templates when `ANTHROPIC_API_KEY` is not set, so the simulator is fully playable without an API key.
