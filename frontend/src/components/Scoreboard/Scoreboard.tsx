...
export default function Scoreboard({ match, recentBalls }: Props) {
  ...
  const score = isBattingTeamB ? match.team_b_score : match.team_a_score
  ...
  const rr = runRate(score, match.current_over, match.current_ball)
  const proj = projected(score, match.current_over, match.current_ball)
      ...
      <div style={s.scoreSection}>
        <div style={s.battingBlock}>
          ...
          <div style={s.scoreRow}>
            <span style={s.mainScore}>{score}</span>
...
const s: Record<string, React.CSSProperties> = {
  ...
  liveDot: { width: 6, height: 6, borderRadius: '50%', background: colors.green, boxShadow: `0 0 6px ${colors.green}` },
  ...
  mainScore: { fontSize: 48, fontWeight: 900, color: colors.orange, lineHeight: 1 },
